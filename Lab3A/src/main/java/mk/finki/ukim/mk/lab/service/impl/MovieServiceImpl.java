...
public class MovieServiceImpl implements MovieService {
    ...
    public List<Movie> searchMovies(String text, double rating) {
        if (text.isEmpty() && rating == 0.0) {
            ...
            return movieRepositoryInterface.findByRatingGreaterThanEqual(rating);
        } else if (rating == 0) {
            return movieRepositoryInterface.findByTitleContainingOrSummaryContaining(text, text);
        } else {
            return movieRepositoryInterface.findByTitleContainingOrSummaryContainingAndRatingGreaterThanEqual(text, text, rating);
        }
    }
